...
- **工作区路径**：E:\liushujian-student-course\java-runtime-principle-one
- **代码作者**：ThinkPad
- **操作系统版本**：Windows 11
- **当前时间**：2025-11-30 13:56:20
...
| **Repository** | 数据库访问与持久化操作             | 使用 `@EntityGraph` 避免 N+1 查询问题     |
...
- 使用 `@Valid` 与 JSR-303 校验注解（如 `@NotBlank`, `@Size` 等）
  - 注意：在 Java 8 中校验注解位于 `javax.validation.constraints.*`
