...
- **工作目录**：`E:\liushujian-student-course\springboot-framework-mybatis-six`
- **操作系统**：Windows 11
- **代码作者**：ThinkPad
- **开发语言**：Java 8
- **构建工具**：Maven
...
```
springboot-framework-mybatis-six
├── src
│   ├── main
│   │   ├── java
│   │   │   └── com
...
| **Service**    | 实现业务逻辑、事务管理与数据校验   | 必须通过 Repository 或 DAO 层访问数据库；返回 DTO 而非 Entity（除非必要） |
| **DAO**        | 数据库访问与持久化操作             | 使用 MyBatis XML 或注解方式访问数据库；避免复杂 SQL 逻辑      |
